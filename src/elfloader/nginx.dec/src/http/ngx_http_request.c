...
ngx_http_init_request(ngx_event_t *rev)
{
	...
	// A potential instrumentation point.
	printf("=> %s:%d: %s: START. pid %d\n", __FILE__, __LINE__, __func__, getpid());

//	if(flag_lmvx) lmvx_start("ngx_http_process_request_line", 1, rev);	// xiaoguang
    rev->handler(rev);
//	if(flag_lmvx) lmvx_end();	// xiaoguang

	printf("=> %s:%d: %s: END. pid %d\n", __FILE__, __LINE__, __func__, getpid());
	// after real function call
}
...
//	if(flag_lmvx) lmvx_start(__func__, 1, rev);	// xiaoguang
	printf("(%d) %s:%d: %s: start. pid %d\n", getpid(), __FILE__, __LINE__, __func__, getpid());
...
//				if(flag_lmvx) lmvx_end();	// xiaoguang
		...
		if(flag_lmvx) lmvx_start("ngx_http_parse_request_line", 2, r, r->header_in);	// xiaoguang
...
ngx_http_process_request(ngx_http_request_t *r)
{
	...
	if(flag_lmvx) lmvx_start("ngx_http_handler", 1, r);	// xiaoguang
    ngx_http_handler(r);
	if(flag_lmvx) lmvx_end();	// xiaoguang

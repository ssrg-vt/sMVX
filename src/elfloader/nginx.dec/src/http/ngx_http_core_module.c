...
//	if(flag_lmvx) lmvx_start(__func__, 1, r);	// xiaoguang
	printf("(%d) %s:%d: %s: START. pid %d. r internal %d\n", getpid(),
			__FILE__, __LINE__, __func__, getpid(), r->internal);
	...
	printf("%s:%d: %s: END. pid %d. r internal %d\n", __FILE__, __LINE__, __func__, getpid(), r->internal);
